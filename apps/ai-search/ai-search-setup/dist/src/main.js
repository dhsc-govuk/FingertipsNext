...
import { buildGeographySearchIndex, buildIndicatorSearchIndex, } from './indexOperations.js';
import { getEnvironmentVariable, getIndicatorsJsonData, } from './utils/helpers.js';
import { createDistrictLevelFromCounty, parseAreaData, } from './utils/areaDocumentHelpers.js';
import { parseIndicatorData } from './utils/indicatorDocumentHelpers.js';
import { AREA_SEARCH_INDEX_NAME, INDICATOR_SEARCH_INDEX_NAME, } from './constants.js';
...
async function main() {
    ...
    await createAndPopulateIndex(indexClient, buildIndicatorSearchIndex, indicatorSearchIndexName, indicatorData);
    ...
    await createAndPopulateIndex(indexClient, buildGeographySearchIndex, areaSearchIndexName, extendedAreaData);
}
main().catch((err) => {
    console.error(err);
});
