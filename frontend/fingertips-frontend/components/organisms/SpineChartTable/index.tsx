...
const getBenchmarkHeader = (
  areaCount: number,
  heading: SpineChartTableHeadingEnum,
  index: number
): ReactNode =>
  areaCount < 2 ? (
    <StyledBenchmarkTrendHeader
...
const getCellHeader = (
  ...
  if (heading === SpineChartTableHeadingEnum.BenchmarkTrend)
    return getBenchmarkHeader(dataLength, heading, index);

  return heading === SpineChartTableHeadingEnum.AreaCount ? (
    <StyledAlignRightHeader
      data-testid={`header-${heading}-${index}`}
      key={`header-${heading}`}
    >
...
export function SpineChartTable({
  ...
  const tableData = healthIndicatorData.map((areaData) =>
    mapToSpineChartTableData(areaData)
  );
  const englandData = englandBenchmarkData
    ? mapToSpineChartTableData(englandBenchmarkData)
    : [];
  const groupData = groupIndicatorData
    ? mapToSpineChartTableData(groupIndicatorData)
    : [];
      ...
      <Table
        head={
          <>
            ...
            <Table.Row>
              <StyledAlignLeftHeader
                data-testid={`header-${SpineChartTableHeadingEnum.AreaPeriod}-${0}`}
                key={`header-${SpineChartTableHeadingEnum.AreaPeriod}`}
              >
                {SpineChartTableHeadingEnum.AreaPeriod}
              </StyledAlignLeftHeader>
              {healthIndicatorData.map(() =>
                Object.values(SpineChartTableHeadingEnum)
                  .filter(
                    (value) => value !== SpineChartTableHeadingEnum.AreaPeriod
                  )
                  .filter(
                    (value) =>
                      value !== SpineChartTableHeadingEnum.BenchmarkValue
                  )
                  .map((heading, index) =>
                    getCellHeader(
                      heading,
              ...
              <StyledGreyHeader
                data-testid={`header-${SpineChartTableHeadingEnum.BenchmarkValue}-${6}`}
              >
                {SpineChartTableHeadingEnum.BenchmarkValue}{' '}
