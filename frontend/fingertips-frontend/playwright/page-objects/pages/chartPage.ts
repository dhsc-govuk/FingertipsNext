  ...
  async checkChartVisibility(
    ...
    for (const visibleComponent of visibleComponents) {
      ...
      await expect(this.page.getByTestId(visibleComponent.componentLocator), {
        message: `Screenshot comparison failed for :${visibleComponent.componentLocator} - you may need to run the update screenshot manual CI job - see directory README.md for details.`,
      }).toHaveScreenshot(
