...
{
    public class DataManager(AreaFetcher areaFetcher, PostcodeFetcher postcodeFetcher)
    {
        private readonly AreaFetcher _areaFetcher = areaFetcher;
        private readonly PostcodeFetcher _postcodeFetcher = postcodeFetcher;

        public async Task CreateAreaDataAsync()
        {
            var areas = await _areaFetcher.FetchAsync();
